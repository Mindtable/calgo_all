...
public:
    ...
    long long balance_by_key(long long value) {
        Node *tile = search(root, value);
        return get_balance(tile);
    }
...
int main() {
    ...
    for (long long i = 0; i < n; i++) {
        std::cout << -set.balance_by_key(values[i]) << "\n";
    }
}
